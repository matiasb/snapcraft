...
class StoreClient():
    ...
    def update_binary_metadata(self, snap_name, metadata, force):
        """Update the metadata in the server."""
        account_info = self.get_account_information()
        series = constants.DEFAULT_SERIES
        try:
        ...
        return self._refresh_if_necessary(
            self.sca.update_binary_metadata, snap_id, snap_name, metadata,
            force)
...
class SCAClient(Client):
    ...
    def update_binary_metadata(self, snap_id, snap_name, metadata, force):
        """Update the binary metadata in SCA."""
